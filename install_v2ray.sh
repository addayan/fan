...
checkv4v6(){
    v6=$(curl -s6m8 ipget.net -k)
    v4=$(curl -s4m8 ipget.net -k)
}
...
if [[ "$(needNginx)" = "no" ]]; then
    if [[ "$TLS" = "true" ]]; then
        while true; do
            ...
            read -p "请输入v2ray监听端口[随机端口100-65535的一个数字，默认随机]（当前随机端口为：$PORT）：" -e PORT_INPUT
            PORT=${PORT_INPUT:-$PORT}
            if [[ $PORT != 80 && $PORT != 22 ]]; then
                break
            else
                echo "端口不能是80或22，请重新输入"
            fi
        done
    else
        PORT=$(shuf -i 200-65000 -n 1)
        read -p "请输入v2ray监听端口[100-65535的一个数字，默认随机]（当前随机端口为：$PORT）：" -e PORT_INPUT
        PORT=${PORT_INPUT:-$PORT}
        if [[ "${PORT:0:1}" = "0" ]]; then
    ...
    PORT=$(shuf -i 200-65000 -n 1)
    read -p "请输入Nginx监听端口[100-65535的一个数字，默认随机]（当前随机端口为：$PORT）：" -e PORT_INPUT
    PORT=${PORT_INPUT:-$PORT}
    if [[ "${PORT:0:1}" = "0" ]]; then
...
menu() {
    ...
    echo -e "  ${GREEN}5.${PLAIN}  停止V2ray"
    echo -e "  ${GREEN}6.${BLUE}  查看V2ray配置链接"
    echo -e "  ${GREEN}7.${PLAIN}  重启nginx"
    echo -e "  ${GREEN}0.${PLAIN}  退出"
    echo -n " 当前状态："
    statusText
    echo 

    read -p " 回车开始安装：" answer
    case ${answer:-1} in
        0)
            exit 0
            ;;
